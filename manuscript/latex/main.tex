...
\title{Physics-Informed Neural Networks for Predicting Caspian Sea Level Variations}

\author{\IEEEauthorblockN{Bekzat Ashirbek\IEEEauthorrefmark{1}}
\IEEEauthorblockN{Arlan Mirseiit\IEEEauthorrefmark{1}}
\IEEEauthorblockN{Amir Nurseit\IEEEauthorrefmark{1}}
\IEEEauthorblockN{Anar Rakhimzhanova\IEEEauthorrefmark{1}}
\IEEEauthorblockA{\IEEEauthorrefmark{1}\textit{Astana IT University}\\
...
\begin{abstract}
We develop three Physics-Informed Neural Network (PINN) formulations --- water-balance, Budyko-constrained, and Priestley--Taylor energy-balance --- for predicting Caspian Sea $\Delta H$ variations, all sharing an identical CNN--LSTM backbone. Across 60 models trained with five random seeds, PINN-3 achieves the highest $R^2$ ($0.6939 \pm 0.0032$) with exceptional stability. Physics constraints reduce seed-to-seed $R^2$ variance by a factor of 2--17$\times$ and enable positive $R^2$ at 10-day resolution where the unconstrained baseline fails. Simple baselines (persistence: $R^2{=}0.017$, climatology: $R^2{=}0.0$) confirm the added value of all neural models. We contribute a reproducible data-integration pipeline and an empirical framework for evaluating physics constraints in closed-basin modeling.
\end{abstract}
...
At 10-day resolution (Appendix), only physics-informed models achieve positive $R^2$ (PINN-1: $0.0827{\pm}0.0302$, PINN-2: $0.1025{\pm}0.0241$), while the baseline remains negative ($-0.1722{\pm}0.0555$).
...
For Caspian Sea forecasting, PINN-3 is recommended. The reproducible pipeline, public dataset~\cite{hersbach2020era5,munoz2021eraland,rodell2004gldas,schwatke2015dahiti,grimaldi2022glofas}, and open-source code at \texttt{https://github.com/fulaura/Caspian-Sea-level-prediction-PINN} provide a foundation for extending physics-aware modeling to other closed-basin hydrological systems.

\section*{Acknowledgment}
The authors thank Dr. Anar Rakhimzhanova for supervision and guidance throughout this research.
...
ERA5, ERA5-Land, GLDAS, DAHITI, and GloFAS data are publicly available~\cite{hersbach2020era5,munoz2021eraland,rodell2004gldas,schwatke2015dahiti,grimaldi2022glofas}. Code and trained models are available at \texttt{https://github.com/fulaura/Caspian-Sea-level-prediction-PINN}.
