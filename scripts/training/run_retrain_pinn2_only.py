...
from pinn2_budyko_river import BudykoRiverPINN2
...
OUT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "results", "retrain_pinn2"))
...
def run_standard_train_eval(model, train_loader, test_loader, full_ds, epochs, lr, dt_val, save_dir, model_type="pinn2", dry_run=False):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.autocast(device_type=DEVICE.type, dtype=torch.float16 if DEVICE.type == "cuda" else torch.bfloat16, enabled=(DEVICE.type == "cuda")):
                ...
                elif model_type == "pinn2_river":
                    pred_dh, beta_volga, n_t, scale_t = model(x)
                    loss_d = F.mse_loss(pred_dh, y_norm)
                    loss_p = model.budyko_river_loss(pred_dh * t_std + t_mean, beta_volga, n_t, scale_t, pl, el, qv, pb, petb, dt_val)
                    prec_d, prec_p = torch.exp(-model.log_vars[0]), torch.exp(-model.log_vars[1])
                    total_loss = 0.5 * (prec_d * loss_d + prec_p * loss_p + model.log_vars.sum()) + 0.1 * torch.pow(beta_volga - 1.0, 2).mean()
...
def run_master_pinn2_suite(dry_run=False):
    total_start = time()
    if dry_run:
    ...
    # 1. Multi-Seed Benchmark for PINN-2 and PINN-2 River Inflow
    for mod_name in ["pinn2", "pinn2_river"]:
        print(f"\n{'='*70}\nEXPERIMENT 1: MULTI-SEED BENCHMARK ({mod_name.upper()})\n{'='*70}", flush=True)
        exp1_dir = os.path.join(OUT_ROOT, ("dryrun_" if dry_run else "") + "1_multiseed", mod_name)
        ...
        for res in ORDERED_RESOLUTIONS:
            ...
            print(f"\nMultiseed {mod_name} -> Res: {res}", flush=True)
            ...
            for s in seed_list:
                set_seed(s)
                if mod_name == "pinn2":
                    mdl = BudykoPINN2_v21(13, seq_len=SEQ_LEN).to(DEVICE)
                else:
                    mdl = BudykoRiverPINN2(13, seq_len=SEQ_LEN).to(DEVICE)
    ...
    # 2. Monte Carlo Dropout Uncertainty (PINN-2 River Inflow)
    print(f"\n{'='*70}\nEXPERIMENT 2: MONTE CARLO DROPOUT UNCERTAINTY BOUNDS (PINN-2 RIVER)\n{'='*70}", flush=True)
    exp2_dir = os.path.join(OUT_ROOT, ("dryrun_" if dry_run else "") + "2_mc_dropout")
    set_seed(42)
    best_p2 = BudykoRiverPINN2(13, seq_len=SEQ_LEN).to(DEVICE)
    _, best_p2 = run_standard_train_eval(best_p2, train_ld_1m, test_ld_1m, full_ds_1m, 250, 3e-4, cfg_1m['dt'], exp2_dir, "pinn2_river", dry_run=dry_run)
    
    best_p2.train() # Enable dropout
    ...
    print(f"\n{'='*70}\nRETRAINING OF PINN2 SATELLITE SUITE COMPLETED in {timedelta(seconds=time() - total_start)}!\n{'='*70}", flush=True)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Retrain PINN-2 Suite")
    parser.add_argument("--dry-run", action="store_true", help="Run 1 epoch smoke test")
    args, _ = parser.parse_known_args()
    run_master_pinn2_suite(dry_run=args.dry_run)
