...
RESOLUTIONS = {
    "1m": {"path": os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "dataset", "merged_grid_dataset_monthly.nc")), "dt": 30.42 * 86400, "epochs": 250, "batch_size": 16, "lr": 3e-4, "lambda_pinn1": 100.0},
    "10d": {"path": os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "dataset", "merged_grid_dataset_10day.nc")), "dt": 10.0 * 86400, "epochs": 150, "batch_size": 32, "lr": 3e-4, "lambda_pinn1": 50.0},
...
class UnifiedMultiSeedDataset(Dataset):
    def __init__(self, ds, volga_data, p_b_s, pet_b_s, p_l_s, e_l_s, seq_len=6, train_end_year=2017):
        ...
        self.p_b, self.pet_b = p_b_s.values.astype(np.float32), pet_b_s.values.astype(np.float32)
        self.p_l, self.e_l = p_l_s.values.astype(np.float32), e_l_s.values.astype(np.float32)

        print("Loading dataset features into memory...", flush=True)
        data_list = [ds[v].values.astype(np.float32) for v in self.vars]
        v_spatial = self.volga_vals[:, None, None] * np.ones_like(data_list[0][0])
        self.data_raw = np.stack(data_list + [v_spatial], axis=1)
        
        water_level = ds.water_level.to_series().ffill().fillna(0)
        delta_h = water_level.diff().fillna(0)
        
        self.targets_raw = delta_h.values.astype(np.float32)
        self.water_level = water_level.values.astype(np.float32)
        ...
        self.data = np.nan_to_num((self.data_raw - self.stats['mean']) / (self.stats['std'] + 1e-8))
        self.targets_norm = np.nan_to_num((self.targets_raw - self.stats['t_mean']) / (self.stats['t_std'] + 1e-8))
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.FloatTensor(x), torch.FloatTensor([y_norm]), torch.FloatTensor([y_raw]),
            ...
            torch.FloatTensor([self.volga_vals[f_idx]]), f_idx
        )

# ── Training Engines per Model Type ───────────────────────────────────────
def train_eval_seed(model_name, seed, full_ds, train_loader, test_loader, cfg, model_dir):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            x, y_norm, y_raw, pb, petb, pl, el, qv, _ = [b.to(DEVICE) for b in batch]
            optimizer.zero_grad()
