...
for s in seeds:
    ...
    if os.path.exists(b_csv):
        ...
        b_levels.append(df_b['Predicted_Level'].values)
        b_deltas.append(df_b['Predicted_DeltaH'].values)
        if actual_level is None:
            actual_level = df_b['Actual_Level'].values
            actual_delta = df_b['Actual_DeltaH'].values

    # PINN-2
    p2_csv = os.path.join(results_master, "1_multiseed", "pinn2", "10d", f"seed_{s}", "prediction_data.csv")
    if os.path.exists(p2_csv):
        df_p = pd.read_csv(p2_csv)
        p2_levels.append(df_p['Predicted_Level'].values)
        p2_deltas.append(df_p['Predicted_DeltaH'].values)

if not b_levels or not p2_levels:
    print("Error: Could not find training outputs for both models.")
    exit(1)

# Find minimum length to align shapes
min_len = min(len(actual_level),
              min(len(x) for x in b_levels),
              min(len(x) for x in p2_levels))

actual_level = actual_level[:min_len]
actual_delta = actual_delta[:min_len]

b_levels = np.array([x[:min_len] for x in b_levels])
b_deltas = np.array([x[:min_len] for x in b_deltas])
p2_levels = np.array([x[:min_len] for x in p2_levels])
p2_deltas = np.array([x[:min_len] for x in p2_deltas])

dates = pd.date_range(start="2018-01-01", periods=min_len, freq="10D")
...
ax1.set_title('10-Day Absolute Sea Level Elevation (H) Predictions (2018–2026)', fontsize=14, fontweight='bold', pad=10)
...
ax2.set_title('10-Day Sea Level Change Dynamics (Delta H) Predictions (2018–2026)', fontsize=14, fontweight='bold', pad=10)
...
plt.tight_layout()
out_plot = os.path.join(figures_dir, "fig4_9_pinn2_10d_dynamics.png")
plt.savefig(out_plot, dpi=300)
plt.close()

print(f"10-Day Dynamics comparison plot successfully saved to {out_plot}!")
