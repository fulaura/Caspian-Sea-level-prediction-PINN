...
TRAIN_END_YEAR = 2017
SEQ_LEN = 6        
OUT_ROOT = "antigravity/results/pinn2/interval_comparison_retrain"
...
HYPARAMS = {
    "1m": {"epochs": 250, "batch_size": 16, "lr": 3e-4},
    "10d": {"epochs": 150, "batch_size": 32, "lr": 3e-4},
    "1d": {"epochs": 30, "batch_size": 128, "lr": 1e-3} # Faster for daily
}
...
def train_resolution(res_name, ds_path):
    ...
    for epoch in range(cfg['epochs']):
        ...
        for batch in train_loader:
            ...
            train_d += loss_data.item()
            train_p += loss_phys.item()
        ...
        avg_p = train_p / len(train_loader)
        avg_val = val_l / len(test_loader)
        
        history['train_total'].append(avg_train)
        history['train_data'].append(avg_d)
        history['train_phys'].append(avg_p)
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(f"Ep {epoch+1:3d}/{cfg['epochs']} | Tot: {avg_train:.4f} | Data: {avg_d:.4f} | Phys: {avg_p:.4f} | Val MSE: {avg_val:.6f}", flush=True)
    ...
    var_actual = np.mean((t_raw_arr - np.mean(t_raw_arr))**2)
    ...
    results = {"rmse": float(rmse), "corr": float(corr), "r2": float(r2), "res": res_name}
    ...
    print(f"\n[{res_name}] PINN-2 Metrics -> RMSE: {rmse:.4f}, Corr: {corr:.4f}, R^2: {r2:.4f}\n", flush=True)

    test_dates = pd.to_datetime(ds.time.values)[[full_ds.valid_indices[i] for i in test_indices]]
    
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(15, 10))
    ax1.plot(test_dates, t_raw_arr, label='Actual Delta H', alpha=0.7, color='#2b8cbe', linewidth=2)
    ax1.plot(test_dates, p_raw_arr, label='PINN-2 Predicted Delta H', color='#e34a33', linewidth=2, linestyle='--')
    ax1.set_title(f"PINN-2 Budyko ({res_name}) - Water Level Change ΔH (RMSE: {rmse:.4f} m, R²: {r2:.4f})", fontsize=14, fontweight='bold')
    ax1.set_xlabel("Year", fontsize=12, fontweight='bold')
    ax1.set_ylabel("Monthly Level Change ΔH (m)", fontsize=12, fontweight='bold')
    ax1.legend(fontsize=11); ax1.grid(True, linestyle=':', alpha=0.6)
    
    ax2.plot(test_dates, actual_abs, label='Actual Water Level (Baltic System)', color='#31a354', linewidth=2.5)
    ax2.plot(test_dates, pred_abs, label='PINN-2 Reconstructed Level', color='#756bb1', linestyle='--', linewidth=2.5)
    ax2.set_title(f"PINN-2 Budyko ({res_name}) - Absolute Water Level Reconstruction", fontsize=14, fontweight='bold')
    ax2.set_xlabel("Year", fontsize=12, fontweight='bold')
    ax2.set_ylabel("Caspian Sea Surface Level (m)", fontsize=12, fontweight='bold')
    ax2.legend(fontsize=11); ax2.grid(True, linestyle=':', alpha=0.6)
    
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "comparison_plots.png"), dpi=300)
    plt.close()
