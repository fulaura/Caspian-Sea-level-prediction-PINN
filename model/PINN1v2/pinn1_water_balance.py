...
class WaterBalancePINN1(nn.Module):
    ...
    def __init__(self, in_channels=13, hidden_dim=64, seq_len=6):
        ...
        # 3. Prediction Head — 4 outputs: dh_pred + 3 physics corrections
    ...
    def water_balance_loss(
        ...
        P_lake,          # precipitation on lake surface     [m / month]
        E_lake,          # ERA5 evaporation from lake        [m / month]  (positive value)
        Q_volga,         # Volga discharge                   [m³ / s]
        P_basin,         # basin precipitation               [m / month]
        PET_basin,       # basin potential ET                [m / month]
    ):
        """
        Soft constraint that drives the network's predicted Δh toward the Δh
        implied by mass conservation.  Everything is in m/month, so the
        residual is in m² — same scale convention as PINN-2's Budyko loss.
        """

        # 1. Lake surface: direct precipitation minus evaporation
        lake_contrib = P_lake - alpha_E * E_lake

        # 2. Volga inflow → m/month over the lake area
        Q_in_m = beta_volga * Q_volga * SEC_PER_MONTH / A_LAKE_M2

        # 3. Residual basin contribution (Ural, Terek, groundwater)
